...
export default function SignInPage() {
  ...
  const handleEmail = async (e) => {
    ...
    setLoading(false);
    if (r?.error) setErr('Email sau parolă incorectă.');
    else { toast.success('Bun venit înapoi!'); router.push(callbackUrl); }
  };
    ...
    <div className="min-h-screen flex items-center justify-center px-4 aurora-bg relative">
      <div className="relative z-10 w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <h1 className="mt-4 font-display text-3xl font-bold">Bine ai venit la <span className="gradient-text">YouthAI OS</span></h1>
          <p className="mt-2 text-sm text-muted-foreground">Autentifică-te pentru a accesa spațiile tale</p>
        </div>

        <div className="rounded-2xl border border-border bg-card/70 backdrop-blur-xl p-6 shadow-xl">
          <button onClick={()=>signIn('google', { callbackUrl })} className="w-full inline-flex items-center justify-center gap-2 rounded-xl border border-border bg-background hover:bg-accent px-4 py-2.5 text-sm font-medium transition">
          ...
          <form onSubmit={handleEmail} className="space-y-3">
            ...
            <label className="flex items-center gap-2 rounded-xl border border-border bg-background px-3 py-2.5 focus-within:ring-2 focus-within:ring-primary">
              ...
              <input type="password" required value={password} onChange={e=>setPassword(e.target.value)} placeholder="Parolă" className="flex-1 bg-transparent text-sm outline-none" />
            </label>
            ...
            <button type="submit" disabled={loading} className="w-full inline-flex items-center justify-center gap-2 rounded-xl bg-primary text-primary-foreground py-2.5 text-sm font-medium hover:opacity-90 disabled:opacity-50">
              {loading && <Loader2 className="w-4 h-4 animate-spin" />} Autentificare
            </button>
          </form>
          <div className="mt-5 text-center text-xs text-muted-foreground">
            Nu ai cont? <Link href="/signup" className="text-primary hover:underline">Creează unul</Link>
          </div>
        </div>
      </div>
    </div>
