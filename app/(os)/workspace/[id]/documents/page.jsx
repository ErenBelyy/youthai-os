...
export default function DocumentsPage({ params }) {
  ...
  const upload = async (e) => {
    ...
    if (r.ok) { toast.success('Document uploaded'); load(); }
  };
  ...
  const summarize = async (doc) => {
    ...
    try {
      const r = await fetch('/api/ai/chat', { method:'POST', headers:{'Content-Type':'application/json'},
        body: JSON.stringify({ messages: [{ role:'user', content: `Summarize this document in 5 bullets, then list 3 action items. Content:\n\n${doc.content||''}` }], workspaceId: wsId }) });
  ...
  return (
    <div>
      <PageHeader icon={FileText} title="Documents" subtitle="Upload PDFs and notes. Index for semantic search. Generate AI summaries."
        actions={
            ...
            <button key={d.id} onClick={()=>setOpenDoc(d)} className="text-left rounded-2xl border border-border bg-card/60 backdrop-blur-xl p-4 hover:border-primary/40 transition">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-[11px] text-muted-foreground">{Math.round((d.size||0)/1024)} KB · {new Date(d.createdAt).toLocaleDateString()}</div>
                </div>
              </div>
          ...
          <div className="w-full max-w-2xl max-h-[85vh] flex flex-col rounded-2xl border border-border bg-card">
            ...
            <div className="flex-1 overflow-y-auto p-4">
              {openDoc.summary ? <ReactMarkdown>{openDoc.summary}</ReactMarkdown>
                : <p className="text-sm text-muted-foreground">No summary yet. Generate one with AI.</p>}
            </div>
            <div className="p-3 border-t border-border flex justify-end">
              <button onClick={()=>summarize(openDoc)} disabled={summarizing} className="inline-flex items-center gap-2 px-3 py-2 rounded-lg bg-primary text-primary-foreground text-sm">
                {summarizing ? <Loader2 className="w-4 h-4 animate-spin" /> : <Sparkles className="w-4 h-4" />} {openDoc.summary?'Re-summarize':'Summarize with AI'}
              </button>
            </div>
          </div>
        </div>
