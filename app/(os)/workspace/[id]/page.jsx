...
import { LayoutDashboard, ArrowRight, FolderKanban, HandCoins, FileText, Bot, Sparkles, MessageSquare } from 'lucide-react';
  ...
  return (
    <div>
      <PageHeader
        ...
        title={ws ? `${ws.emoji} ${ws.name}` : 'Workspace'}
        subtitle={ws?.tagline || 'Loading workspace…'}
        actions={
      ...
      <div className="p-6 md:p-8 max-w-7xl mx-auto space-y-6">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          ...
          <Stat label="Grants" value={stats.grants} icon={HandCoins} />
          <Stat label="Documents" value={stats.documents} icon={FileText} />
          <Stat label="Opportunities" value={stats.opportunities} icon={Sparkles} />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          <div className="lg:col-span-2 rounded-2xl border border-border bg-card/60 backdrop-blur-xl p-5">
            <div className="flex items-center justify-between">
              ...
              <Link href={`/workspace/${wsId}/projects`} className="text-xs text-primary inline-flex items-center gap-1 hover:underline">All projects <ArrowRight className="w-3 h-3" /></Link>
            </div>
